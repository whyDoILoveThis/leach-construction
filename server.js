...
app.post('/server', async (req, res) => {
    try {
        const {special, service, type, name, email, phone, message } = req.body;
        ...
        const msg = {
            ...
            subject: `${newType === 'New Message' ? `📨 ${name}` :
             special === `${newType} Customer Service` ? `👷‍♂️ ${name}` :
              `💰💲 ${name}` }`,
            ...
            <body>
                <div class="container">
                    <h1>${newType === 'New Message' ? newType :
                     special === `${newType} Customer Service` ? `${newType} Customer Service Request` :
